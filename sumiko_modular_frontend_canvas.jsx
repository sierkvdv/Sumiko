...
/**
 * SUMIKO – Modular Frontend (Canvas Preview)
 * -------------------------------------------------------------
 * Clean, modular UI with CLEAR BACKEND HOOKS for Cursor.
 * Now includes a full cart flow: add → edit quantities → checkout form (stub).
 * Also keeps the subtle "steam" + "breathing light" animation in the hero.
 */

// ---------------------------
// Design Tokens (could move to CSS variables / Tailwind config)
// ---------------------------
const COLORS = {
  primary: "#A27C48",
  beige: "#DDD6CE",
...
// ---------------------------
// API SERVICE STUBS (for Cursor to replace)
// ---------------------------
const Api = {
  /** BACKEND HOOK: fetch teas list from server */
  async fetchTeas() {
    // return fetch("/api/teas").then(r => r.json())
    return Promise.resolve(INITIAL_TEAS);
  },
  /** BACKEND HOOK: fetch single tea by id */
  async fetchTeaById(id) {
    // return fetch(`/api/teas/${id}`).then(r => r.json())
    const all = await Api.fetchTeas();
    return all.find((t) => t.id === id);
  },
  /** BACKEND HOOK: start checkout server-side */
  async startCheckout(payload) {
    // return fetch("/api/checkout", { method: "POST", body: JSON.stringify(payload) }).then(r=>r.json())
    return Promise.resolve({ ok: true, checkoutId: "dev-checkout-123" });
  },
};

// ---------------------------
// Cart Store (Context) – local state with clear BACKEND HOOKS
// ---------------------------
...
function CartProvider({ children }) {
  const [items, setItems] = useState([]); // [{id, name, priceCents, qty, image}]

  // Persist locally so refresh keeps cart (optional, easy to replace later)
  useEffect(() => {
    try {
  ...
  const shippingCents = subtotal > 5000 || items.length === 0 ? 0 : 495; // free over €50
...
function Hero({ onScrollToTeas }) {
  return (
    <section className="relative overflow-hidden rounded-2xl border border-black/5 bg-[#F3EFE6] p-8 md:p-12 shadow-lg">
      {/* Inline keyframes for Canvas preview */}
      <style>{`
...
function Journal() {
  // BACKEND HOOK: replace with CMS posts (e.g., /api/posts)
  const posts = [
...
function Contact() {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    const res = await Api.subscribe(email); // BACKEND HOOK
    setOk(!!res?.ok);
  };
...
// ---------------------------
// Product Detail Modal (uses Cart)
// ---------------------------
function ProductModal({ open, teaId, onClose }) {
  ...
  useEffect(() => {
    ...
    Api.fetchTeaById(teaId).then(setTea);
  }, [open, teaId]);
  if (!open) return null;
...
// ---------------------------
// Cart Drawer & Checkout (frontend only; backend hooks marked)
// ---------------------------
...
function CheckoutModal({ onClose }) {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    // BACKEND HOOK: send order/cart + customer details
    ...
    const res = await Api.startCheckout(payload);
    setOk(res?.ok);
    setLoading(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-xl bg-white rounded-2xl p-6 shadow-xl">
        ...
        {ok && <div className="mt-3 text-green-700 text-sm">Order created (dev)! Replace with real gateway.</div>}
      </div>
    </div>
  );
}
...
if (typeof window !== "undefined") {
  ...
  Api.fetchTeaById("longjing").then((t) => console.assert(!!t && t.id === "longjing", "fetchTeaById failed"));
}
